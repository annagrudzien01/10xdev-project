...
System zarządzania sesjami gry umożliwia śledzenie sesji rozgrywki dla profili dzieci. Każdy profil może mieć wiele sesji, ale tylko jedna może być aktywna w danym czasie. Sesje są automatycznie zamykane podczas tworzenia nowej sesji lub mogą być zamykane ręcznie.

### Endpoints:
1. **POST /api/profiles/{profileId}/sessions** - Rozpocznij nową sesję (automatycznie dezaktywuje poprzednią)
2. **PATCH /api/sessions/{sessionId}/end** - Zakończ bieżącą sesję
3. **GET /api/profiles/{profileId}/sessions** - Wyświetl listę sesji z opcjonalnym filtrem `active=true`

### Kluczowe funkcjonalności:
- Automatyczna dezaktywacja poprzednich sesji przy tworzeniu nowej
- Tylko jedna aktywna sesja na profil dziecka
- Paginacja listy sesji
- Filtrowanie sesji aktywnych
...
### 2.2 PATCH /api/sessions/{sessionId}/end
...
- Sesja nie może być już zakończona (`ended_at` musi być NULL)

---

### 2.3 GET /api/profiles/{profileId}/sessions
...
export interface SessionStartDTO {
  ...
  isActive: boolean;        // Czy sesja jest aktywna (zawsze true dla nowo utworzonej)
}
```
...
export interface SessionDTO {
  ...
  isActive: boolean;        // Czy sesja jest aktywna
  startedAt: string;        // ISO timestamp rozpoczęcia
  endedAt: string | null;   // ISO timestamp zakończenia (null jeśli aktywna)
...
### 4.2 PATCH /api/sessions/{sessionId}/end
...
### 4.3 GET /api/profiles/{profileId}/sessions
...
4. SessionService.startSession(profileId, user.id)
   ↓
   SQL: INSERT INTO sessions (child_id, is_active, started_at)
        VALUES ($1, TRUE, now())
        RETURNING *
   
   - Trigger `deactivate_last_session()` automatycznie ustawia `is_active = FALSE`
     dla poprzedniej aktywnej sesji tego dziecka
   - Unique constraint `ux_active_session_per_child` zapewnia max 1 aktywną sesję
...
**SQL Query (automatyczne przez trigger):**
```sql
-- 1. Dezaktywacja poprzedniej sesji (trigger)
UPDATE sessions 
SET is_active = FALSE, ended_at = now() 
WHERE child_id = $1 AND is_active = TRUE;

-- 2. Utworzenie nowej sesji (INSERT)
INSERT INTO sessions (child_id, is_active, started_at)
VALUES ($1, TRUE, now())
RETURNING id, child_id, is_active, started_at, ended_at, created_at, updated_at;
```

---

### 5.2 PATCH /api/sessions/{sessionId}/end - Zakończ sesję
...
3. SessionService.endSession(sessionId, user.id)
   ...
   a) Weryfikacja własności:
      ...
      - Jeśli brak rekordu → throw NotFoundError
      - Jeśli parent_id ≠ user.id → throw ForbiddenError
      - Jeśli ended_at IS NOT NULL → throw ValidationError ("Session is already ended")
...
### 5.3 GET /api/profiles/{profileId}/sessions - Lista sesji
...
4. SessionService.listSessions(profileId, user.id, params)
   ...
   SQL: SELECT * FROM sessions
        WHERE child_id = $1
        [AND is_active = $2 IF params.active IS NOT NULL]
...
SELECT id, child_id, is_active, started_at, ended_at, created_at, updated_at
FROM sessions
WHERE child_id = $1
  [AND is_active = $2]  -- opcjonalny filtr
ORDER BY started_at DESC
LIMIT $pageSize OFFSET $offset;
...
- `active`: musi być "true" lub "false" (string transformowany do boolean)

### 6.5 Business Logic Security

**POST sessions:**
- Trigger `deactivate_last_session()` automatycznie zamyka poprzednią sesję
- Unique constraint `ux_active_session_per_child` zapobiega wielu aktywnym sesjom
- Nie można rozpocząć sesji dla nieistniejącego profilu (404)

**PATCH end session:**
- Nie można zakończyć już zakończonej sesji (400)
- Sprawdzenie `ended_at IS NULL` przed aktualizacją
...
| Trigger nie zadziałał (rzadkie)  | 500 | `internal_error`    | "An unexpected error occurred"         | SessionService        |

---

### 7.2 Macierz błędów dla PATCH /sessions/{sessionId}/end
...
**Istniejący indeks:**
```sql
-- Unique constraint zapewniający max 1 aktywną sesję na dziecko
CREATE UNIQUE INDEX ux_active_session_per_child 
ON sessions(child_id) 
WHERE is_active = TRUE;
```

**Zalecane dodatkowe indeksy:**
```sql
-- Indeks dla szybkiego wyszukiwania sesji dziecka (GET sessions)
CREATE INDEX idx_sessions_child_id ON sessions(child_id);

-- Indeks dla filtrowania aktywnych sesji + sortowania
CREATE INDEX idx_sessions_child_active_started 
ON sessions(child_id, is_active, started_at DESC);
...
**Uzasadnienie:**
- `idx_sessions_child_id`: Przyspiesza `WHERE child_id = $1` w GET sessions
- `idx_sessions_child_active_started`: Optymalizuje filtrowanie + sortowanie (composite index)
...
**POST sessions:**
- Single INSERT z RETURNING → 1 round-trip do DB
- Trigger działa automatycznie (nie wymaga dodatkowego SELECT/UPDATE)
- Koszt: ~2-5ms
...
- Można zoptymalizować do pojedynczego UPDATE z RETURNING + JOIN:
  ...
  WHERE s.id = $1 AND s.child_id = cp.id AND cp.parent_id = $2 AND s.ended_at IS NULL
  RETURNING s.*;
  ```
- Koszt: ~3-8ms
...
1. **Active session per profile** (GET with `active=true`)
   - TTL: 5 minut
   - Invalidacja: po POST/PATCH sessions
...
**Nie cache'ować:**
- Pełna lista sesji (często się zmienia)
...
| -------------------------------- | ------------------ | ---------------------------------------- |
| Trigger `deactivate_last_session`| Niskie             | Już zoptymalizowany (WHERE is_active)    |
| COUNT(*) dla paginacji           | Średnie            | Index + cache count                      |
| OFFSET dla dużych zbiorów        | Średnie            | Cursor-based pagination (przyszłość)     |
| Równoczesne POST sessions        | Niskie             | Unique constraint zapobiega race         |
...
**Zadanie:**
1. Zaimportować typy: `SupabaseClient`, `SessionDTO`, `SessionStartDTO`, `PaginatedResponse`, `SessionListParams`
2. Zaimportować helper functions: `toSessionDTO`, `toSessionStartDTO`
...
6. Zaimplementować metodę `endSession(sessionId: string, parentId: string): Promise<void>`
7. Zaimplementować metodę `listSessions(profileId: string, parentId: string, params: SessionListParams): Promise<PaginatedResponse<SessionDTO>>`
...
```bash
1. POST /api/profiles/{profileId}/sessions → session1
2. POST /api/profiles/{profileId}/sessions → session2
3. GET /api/profiles/{profileId}/sessions?active=true

Expected: Tylko session2 jest aktywna, session1 ma ended_at
```
...
**Test 7: Nie można zakończyć już zakończonej sesji**
```bash
1. PATCH /api/sessions/{sessionId}/end → 200 OK
...
**SQL:**
```sql
-- Index for fast child_id lookups
CREATE INDEX IF NOT EXISTS idx_sessions_child_id 
ON sessions(child_id);

-- Composite index for filtering active sessions + sorting
CREATE INDEX IF NOT EXISTS idx_sessions_child_active_started 
ON sessions(child_id, is_active, started_at DESC);
 ...
 * - Trigger `deactivate_last_session()` handles session closure
 * 
...
1. **3 endpointy REST API:**
   - POST /profiles/{profileId}/sessions - Rozpoczęcie sesji
   - PATCH /sessions/{sessionId}/end - Zakończenie sesji
   - GET /profiles/{profileId}/sessions - Lista sesji z filtrowaniem
...
5. **Wydajność:**
   - Indeksy bazy danych dla fast lookups
...
**Szacowany czas implementacji:** 4-6 godzin (1 dzień roboczy)

**Priorytet:** Wysoki (wymagane dla funkcjonalności gry)

**Zależności:**
- ProfileService (istniejący)
- Typy SessionDTO, SessionStartDTO (istniejące)
