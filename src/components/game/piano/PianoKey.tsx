...
function PianoKeyComponent({ note, label, color, isHighlighted, isDisabled, onClick, onPlaySound }: PianoKeyProps) {
  ...
  const blackKeyClasses = `
    ...
    bg-gray-900 border-2 border-gray-950
    rounded-b-md
    shadow-lg
    flex flex-col items-center justify-end
    pb-1 md:pb-2
  ...
  // Klasy dla stanów
  const stateClasses = `
    ${isHighlighted ? (color === "white" ? "bg-yellow-200 border-yellow-400" : "bg-yellow-600 border-yellow-700") : ""}
    ${isPressed ? "scale-95 brightness-90" : ""}
    ${isDisabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:brightness-110 active:scale-95 active:brightness-90"}
  `;
